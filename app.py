    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(raw_bytes)) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                if not tables:
                    tables = page.extract_tables({"vertical_strategy": "text","horizontal_strategy": "text","snap_tolerance": 5})
                for table in tables:
                    for row in table:
        ...
        if not all_rows:
            if not has_text:
                return None, (f"PDF scanné détecté ({n_pages} page(s)) — ce type de PDF contient des images "
...
def sauvegarder_erb(entreprise, licence_code, periode, xlsx_bytes, sr_rb, sr_cp, ok):
    ...
    try:
        ...
        data = {"entreprise": entreprise, "licence_code": licence_code, "periode": periode,
                "sr_rb": float(sr_rb), "sr_cp": float(sr_cp), "equilibre": bool(ok), "xlsx_base64": xlsx_b64}
...
def charger_historique_erb(licence_code, limit=20):
    ...
    try:
        result = (sb.table("erb_historique")
            ...
            .eq("licence_code", licence_code).order("date_creation", desc=True).limit(limit).execute())
...
def telecharger_erb_sauvegarde(erb_id):
    sb = get_supabase()
    if sb is None: return None, None
    try:
        import base64
        result = (sb.table("erb_historique").select("xlsx_base64,periode,entreprise").eq("id", erb_id).single().execute())
        if result.data:
            xlsx_bytes = base64.b64decode(result.data["xlsx_base64"])
            fname = f"ERB_{result.data['entreprise']}{result.data['periode'].replace(' ','')}.xlsx"
...
st.markdown("""<style>
.stApp { background: linear-gradient(135deg, #0F1B2D 0%, #1A2E4A 40%, #0D2137 70%, #162032 100%) !important; background-attachment: fixed !important; }
.main .block-container { background: rgba(255,255,255,0.04) !important; border-radius: 16px !important; border: 1px solid rgba(255,255,255,0.08) !important; backdrop-filter: blur(10px) !important; padding: 2rem 2.5rem !important; margin-top: 0.5rem !important; }
.stApp, .stApp p, .stApp label, .stApp div { color: #E8EDF4 !important; }
.stApp h1, .stApp h2, .stApp h3 { color: #FFFFFF !important; }
section[data-testid="stSidebar"] > div:first-child { padding-top: 1rem; }
section[data-testid="stSidebar"] { background: linear-gradient(180deg, #1A237E 0%, #283593 30%, #1565C0 65%, #0D47A1 100%) !important; border-right: 1px solid rgba(100,160,255,0.2) !important; }
...
section[data-testid="stSidebar"] button { background: rgba(255,255,255,0.08) !important; border: 1px solid rgba(100,160,255,0.25) !important; border-radius: 10px !important; color: #E3F2FD !important; }
section[data-testid="stSidebar"] button:hover { background: rgba(100,160,255,0.2) !important; border-color: rgba(100,160,255,0.5) !important; }
section[data-testid="stSidebar"] button[kind="primary"] { background: linear-gradient(135deg, #1976D2, #42A5F5) !important; border-color: #64B5F6 !important; font-weight: 700 !important; box-shadow: 0 4px 15px rgba(66,165,245,0.4) !important; }
section[data-testid="stSidebar"] button p { color: white !important; }
.stButton button[kind="primary"] { background: linear-gradient(135deg, #1565C0, #1976D2, #42A5F5) !important; border: none !important; border-radius: 10px !important; color: white !important; font-weight: 600 !important; box-shadow: 0 4px 20px rgba(66,165,245,0.4) !important; }
...
.stButton button[kind="secondary"] { background: rgba(255,255,255,0.07) !important; border: 1px solid rgba(255,255,255,0.18) !important; border-radius: 10px !important; color: #B0C4DE !important; }
.stButton button[kind="secondary"]:hover { background: rgba(255,255,255,0.13) !important; color: white !important; }
.stTextInput input, .stNumberInput input { background: rgba(10,25,50,0.7) !important; border: 1.5px solid rgba(100,160,255,0.4) !important; border-radius: 8px !important; color: #FFFFFF !important; font-weight: 500 !important; font-size: 14px !important; }
.stTextInput input:focus, .stNumberInput input:focus { border-color: #42A5F5 !important; box-shadow: 0 0 0 3px rgba(66,165,245,0.25) !important; background: rgba(10,30,60,0.85) !important; }
.stTextInput label, .stNumberInput label, .stSelectbox label { color: #90CAF9 !important; font-weight: 600 !important; font-size: 13px !important; }
[data-testid="metric-container"] { background: linear-gradient(135deg, rgba(21,101,192,0.25), rgba(30,50,100,0.35)) !important; border: 1px solid rgba(100,160,255,0.2) !important; border-radius: 14px !important; padding: 1rem 1.2rem !important; box-shadow: 0 4px 20px rgba(0,0,0,0.3) !important; }
...
.streamlit-expanderHeader { background: rgba(255,255,255,0.06) !important; border: 1px solid rgba(100,160,255,0.18) !important; border-radius: 10px !important; color: #B0C4DE !important; font-weight: 600 !important; }
.streamlit-expanderContent { background: rgba(255,255,255,0.03) !important; border: 1px solid rgba(100,160,255,0.12) !important; border-top: none !important; border-radius: 0 0 10px 10px !important; }
...
SENDGRID_API_KEY = ""
EMAIL_EXPEDITEUR = "sowmarieta013@gmail.com"
NOM_EXPEDITEUR = "ERB Manager"

LICENCES = {
    "ERB-4C11-316D-54AB": {"entreprise": "Alliance — Amadou Lamine MBODJ", "domaines": ["@Alliance-ac.sn"], "actif": True},
    "ERB-3D3A-9BC8-037B": {"entreprise": "Alliance — Khardiatou DIALLO KA", "domaines": ["@Alliance-ac.sn"], "actif": True},
    "ERB-AA5D-5BA6-58B1": {"entreprise": "Alliance — Mamadou DIALLO", "domaines": ["@Alliance-ac.sn"], "actif": True},
    "ERB-1FF5-8FBA-FC26": {"entreprise": "Client 4", "domaines": ["@gmail.com"], "actif": True},
    "ERB-B987-B122-E9BE": {"entreprise": "Client 5", "domaines": ["@gmail.com"], "actif": True},
}
...
def _send_otp(email, otp, entreprise):
    ...
    html_body = f"""<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0F1B2D;border-radius:12px;padding:32px;color:#E8EDF4">
  <div style="text-align:center;margin-bottom:24px"><div style="font-size:22px;font-weight:800;color:#fff">ERB Manager</div></div>
  ...
  <div style="background:#1A2E4A;border:1px solid #42A5F5;border-radius:12px;text-align:center;padding:20px;margin:24px 0">
  ...
  <p style="font-size:12px;color:#546E7A">Ne partagez jamais ce code.</p>
    ...
    if resend_key and not resend_key.startswith("re.VOTRE"):
        try:
            payload = _json.dumps({"from": f"{NOM_EXPEDITEUR} <{EMAIL_EXPEDITEUR}>", "to": [email],
                "subject": f"[ERB Manager] Votre code : {otp}", "html": html_body}).encode("utf-8")
        ...
        except urllib.error.HTTPError as e: resend_err = f"Resend HTTP {e.code}: {e.read().decode('utf-8','ignore')[:150]}"
        except Exception as ex: resend_err = str(ex)
    api_key = SENDGRID_API_KEY
    try: api_key = st.secrets.get("SENDGRID_API_KEY", api_key)
    except Exception: pass
    if api_key.startswith("SG.VOTRE") or not api_key:
        return False, "Aucun service email configuré (SendGrid ou Resend). Voir la configuration."
    html_body2 = f"""<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0F1B2D;border-radius:12px;padding:32px;color:#E8EDF4">
  <div style="text-align:center;margin-bottom:24px"><div style="font-size:22px;font-weight:800;color:#fff">ERB Manager</div></div>
  <p style="color:#90CAF9;margin-bottom:8px">Bonjour,</p>
  <p style="color:#B0C4DE">Voici votre code de connexion pour <strong style="color:#fff">{entreprise}</strong> :</p>
  <div style="background:#1A2E4A;border:1px solid #42A5F5;border-radius:12px;text-align:center;padding:20px;margin:24px 0">
    <div style="font-size:40px;font-weight:900;letter-spacing:12px;color:#42A5F5;font-family:monospace">{otp}</div>
    <div style="font-size:12px;color:#78909C;margin-top:8px">Valable 10 minutes — usage unique</div>
  </div>
  <p style="font-size:12px;color:#546E7A">Si vous n'avez pas demandé ce code, ignorez cet email.<br>Ne partagez jamais ce code.</p>
  <hr style="border-color:#1A2E4A;margin:24px 0">
  <p style="font-size:11px;color:#37474F;text-align:center">ERB Manager — Rapprochement Bancaire Professionnel</p>
</div>"""
    payload = _json.dumps({"personalizations": [{"to": [{"email": email}]}],
        "from": {"email": EMAIL_EXPEDITEUR, "name": NOM_EXPEDITEUR},
        "subject": f"[ERB Manager] Votre code : {otp}",
        "content": [{"type": "text/html", "value": html_body2}]}).encode("utf-8")
    try:
        req = urllib.request.Request("https://api.sendgrid.com/v3/mail/send", data=payload,
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, method="POST")
        with urllib.request.urlopen(req, context=ctx, timeout=15) as resp:
            if resp.status in (200, 202): return True, ""
            return False, f"SendGrid statut {resp.status}"
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="ignore")
        if e.code == 403 and "allowlist" in body:
            return False, "SendGrid bloqué par Streamlit Cloud. Utilisez Resend.com à la place."
        return False, f"SendGrid HTTP {e.code} : {body[:200]}"
    except Exception as ex:
        return False, str(ex)

def check_licence():
    if st.session_state.get("_licence_ok"): return True
    st.markdown("""<style>
...
.main .block-container { max-width:500px !important; margin:2.5rem auto !important;
    ...
    st.markdown("""<div style="text-align:center;margin-bottom:1.8rem">
  ...
  <div style="background:rgba(255,255,255,0.08);border-radius:4px;height:4px">
    <div style="background:linear-gradient(90deg,#1565C0,#42A5F5);width:{pct};height:4px;border-radius:4px;transition:width 0.3s"></div>
  </div>
</div>""", unsafe_allow_html=True)
    if step == 1:
        ...
        entreprise = info["entreprise"]; domaines = info["domaines"]
        dom_str = "  ·  ".join(domaines)
        st.markdown(f"""<div style="background:rgba(66,165,245,0.1);border:1px solid rgba(66,165,245,0.25);border-radius:10px;padding:10px 14px;margin-bottom:1.2rem">
  ...
  <div style="font-size:11px;color:#607D8B;margin-top:2px">Domaines : {dom_str}</div>
        ...
        email = st.text_input("📧 Votre email professionnel", placeholder=f"prenom.nom{domaines[0]}")
        c1, c2 = st.columns([1, 2])
        with c1:
            if st.button("← Retour", use_container_width=True):
                for k in ["_auth_step","_auth_code_tmp","_auth_info_tmp"]: st.session_state.pop(k, None)
                st.rerun()
        with c2:
            if st.button("📩 Recevoir le code", type="primary", use_container_width=True):
                ...
                if "@" not in email_clean or "." not in email_clean.split("@")[-1]:
                    ...
                    st.session_state["_auth_send_err"] = "" if ok else err
                    st.rerun()
    elif step == 3:
        ...
        if remaining <= 0:
            ...
            for k in ["_auth_step","_auth_code_tmp","_auth_info_tmp","_auth_email","_auth_otp","_auth_otp_time"]:
                st.session_state.pop(k, None)
            st.rerun()
        st.markdown(f"""<div style="background:rgba(29,158,117,0.1);border:1px solid rgba(29,158,117,0.25);border-radius:10px;padding:10px 14px;margin-bottom:1.2rem">
  ...
  <div style="font-size:11px;color:#607D8B;margin-top:2px">⏱️ Expire dans {remaining // 60}m {remaining % 60:02d}s</div>
</div>""", unsafe_allow_html=True)
        if st.session_state.get("_auth_debug_otp"):
            ...
            if send_err:
                ...
                border_color = "#FF9800"; bg_color = "rgba(255,152,0,0.15)"
                note = f'<div style="font-size:11px;color:#888;margin-top:8px">Erreur : {send_err}</div>'
            else:
                label = "🔐 Votre code de connexion :"
                border_color = "#42A5F5"; bg_color = "rgba(66,165,245,0.1)"
                note = '<div style="font-size:11px;color:#90CAF9;margin-top:8px">Vérifiez aussi votre boîte email (et les spams)</div>'
            st.markdown(f"""<div style="background:{bg_color};border:1px solid {border_color};border-radius:10px;padding:14px 18px;margin-bottom:1rem">
  <div style="font-size:12px;color:#FFB74D;font-weight:700;margin-bottom:6px">{label}</div>
  <div style="font-size:36px;font-weight:900;letter-spacing:10px;color:#FFF;font-family:monospace;text-align:center;background:rgba(0,0,0,0.3);border-radius:8px;padding:12px">{st.session_state["_auth_debug_otp"]}</div>
  {note}
</div>""", unsafe_allow_html=True)
        otp_input = st.text_input("🔢 Code à 6 chiffres", placeholder="123456", max_chars=6)
        c1, c2 = st.columns([1, 2])
        with c1:
            if st.button("← Retour", use_container_width=True):
                for k in ["_auth_step","_auth_code_tmp","_auth_info_tmp","_auth_email","_auth_otp","_auth_otp_time"]:
        ...
        with c2:
            if st.button("✅ Se connecter", type="primary", use_container_width=True):
                if otp_input.strip() == st.session_state["_auth_otp"]:
                    ...
                    for k in ["_auth_step","_auth_code_tmp","_auth_info_tmp","_auth_email","_auth_otp","_auth_otp_time","_auth_debug_otp","_auth_send_err"]:
                        st.session_state.pop(k, None)
                    st.rerun()
                else:
    ...
    st.markdown("""<div style="text-align:center;margin-top:1.5rem;font-size:12px;color:#37474F">
...
DEFAULTS = dict(
    ...
    carryover_rb=[], carryover_cp=[], historique=[], regularisees=[],
    rematch_needed=False, _rematching=False, _rematch_msg='',
    inf_cl='', inf_bq='', inf_co='', inf_pe='', s_rb=0.0, s_cp=0.0,
)
...
def pn(v):
    if v is None or (isinstance(v, float) and np.isnan(v)) or str(v).strip() in ('','nan','None','-','—'): return 0.0
...
def detect_header_row(df_raw):
    for i, row in df_raw.iterrows():
        if row.dropna().apply(lambda x: str(x).strip() != '').sum() >= 3: return i
    return 0
...
def _extract_pdf_scanned_via_claude(uploaded_file):
    try:
        ...
        if not api_key: return None, "Clé ANTHROPIC_API_KEY non configurée dans secrets.toml"
...
Ignore les lignes de solde, totaux, en-têtes. Retourne UNIQUEMENT un tableau JSON valide, rien d'autre."""
        payload = json.dumps({"model": "claude-sonnet-4-6", "max_tokens": 4000,
        ...
        req = urllib.request.Request("https://api.anthropic.com/v1/messages", data=payload,
            headers={"x-api-key": api_key, "anthropic-version": "2023-06-01",
                     "content-type": "application/json", "anthropic-beta": "pdfs-2024-09-25"}, method="POST")
        ...
        if not rows: return None, "Aucune transaction détectée dans le relevé"
        df = pd.DataFrame(rows)
        for col in ["date","libelle","piece","debit","credit"]:
            if col not in df.columns: df[col] = "" if col in ["date","libelle","piece"] else 0.0
...
def load_file(uploaded_file):
    ...
    if name.endswith('.csv'):
        for enc in ['utf-8','latin-1','cp1252']:
            try: uploaded_file.seek(0); return pd.read_csv(uploaded_file, encoding=enc, header=None, dtype=str)
            except: pass
...
def apply_map(df_raw, col_map):
    ...
    for _, row in df_raw.iterrows():
        ...
        lib = str(row.get(col_map.get('lib',''),'')).strip() if 'lib' in col_map else ''
        if is_excluded(lib):
        ...
        piece = str(row.get(col_map.get('piece',''),'')).strip() if 'piece' in col_map else ''
...
def _run_match(rb, cp, inversion):
    ...
    for i, rbr in rb.iterrows():
        ...
        if len(candidates) == 1: chosen_j = candidates[0][0]
        else:
...
def auto_match(rb_df, cp_df, inversion=True):
    ...
    if has_carried_rb:
        ...
        rb_carry = pd.DataFrame(columns=rb_df.columns if len(rb_df) else ['date','lib','piece','debit','credit','matched','match_id'])
    ...
    if has_carried_cp:
        ...
        cp_carry = pd.DataFrame(columns=cp_df.columns if len(cp_df) else ['date','lib','piece','debit','credit','matched','match_id'])
    p0_cnt = 0; cp_reserved_p0 = set(); cp_carry_reserved = set()
    if len(cp_carry) > 0:
        cp_carry['matched'] = False; cp_carry['match_id'] = None
        for ci in cp_carry.index:
            carry_d = cp_carry.at[ci, 'debit']; carry_c = cp_carry.at[ci, 'credit']
            if carry_d == 0 and carry_c == 0: continue
            for ri in cp_courant.index:
                if ri in cp_reserved_p0: continue
                cur_d = cp_courant.at[ri, 'debit']; cur_c = cp_courant.at[ri, 'credit']
                if (carry_c > 0 and abs(carry_c - cur_c) <= 1) or (carry_d > 0 and abs(carry_d - cur_d) <= 1):
                    mid = f"CP0_{ci}_{ri}"
                    cp_carry.at[ci, 'matched'] = True; cp_carry.at[ci, 'match_id'] = mid
                    cp_courant.at[ri, 'matched'] = True; cp_courant.at[ri, 'match_id'] = mid
                    cp_reserved_p0.add(ri); cp_carry_reserved.add(ci)
                    p0_cnt += 1; break
    cp_carry_non_reg = cp_carry[~cp_carry['matched'].fillna(False)].copy() if len(cp_carry) > 0 else pd.DataFrame(columns=cp_df.columns)
    cp_df_for_match = pd.concat([cp_courant[~cp_courant['matched'].fillna(False)], cp_carry_non_reg], ignore_index=True)
    rb_carry_out = rb_carry.copy(); p1_cnt = 0; rb_reserved = set()
    if len(rb_carry_out) > 0:
        rb_carry_out['matched'] = False; rb_carry_out['match_id'] = None
        mc1 = st.session_state.mc
        for ci in rb_carry_out.index:
            carry_d = rb_carry_out.at[ci,'debit']; carry_c = rb_carry_out.at[ci,'credit']
            if carry_d == 0 and carry_c == 0: continue
            for ri in rb_courant.index:
                if ri in rb_reserved: continue
                cur_d = rb_courant.at[ri,'debit']; cur_c = rb_courant.at[ri,'credit']
    ...
    if len(rb_carry_out) > 0:
        ...
        for ci in rb_carry_out.index:
            ...
            for cj in cp_tmp.index:
                ...
                cp_d = cp_tmp.at[cj, 'debit']; cp_c = cp_tmp.at[cj, 'credit']
                matched_inv = (carry_d > 0 and abs(carry_d - cp_c) <= 1) or (carry_c > 0 and abs(carry_c - cp_d) <= 1)
                if matched_inv:
                    mc1b += 1; mid = f"P{mc1b}"
                    rb_carry_out.at[ci, 'matched'] = True; rb_carry_out.at[ci, 'match_id'] = mid
                    cp_tmp.at[cj, 'matched'] = True; cp_tmp.at[cj, 'match_id'] = mid
                    cp_reserved.add(cj); p1_cnt += 1; break
    ...
    parts = [rb_p2]
    if len(rb_carry_non_reg) > 0: parts.append(rb_carry_non_reg)
    rb_out = pd.concat(parts, ignore_index=True)
    cp_out = cp_p2
    total = p1_cnt + p2_cnt
    mc = st.session_state.mc
    all_ids = set(rb_out[rb_out['matched']]['match_id'].tolist() + cp_out[cp_out['matched']]['match_id'].tolist())
...
def get_rb_with_carryover():
    rb = st.session_state.rb_df; carryover = st.session_state.carryover_rb
...
def get_cp_with_carryover():
    cp = st.session_state.cp_df; carryover = st.session_state.carryover_cp
    ...
    if 'carry_from' in rb_matched.columns:
        for _, r in rb_matched.iterrows():
            if _is_report(r.to_dict()) and r.get('matched'):
                ...
                regularisees.append({'type': 'RB régularisé', 'from': r.get('carry_from',''), 'date': r.get('date',''),
                    'lib': r.get('lib',''), 'debit': r.get('debit',0), 'credit': r.get('credit',0),
    ...
    if 'carry_from' in cp_matched.columns:
        for _, r in cp_matched.iterrows():
            if _is_report(r.to_dict()) and r.get('matched'):
                ...
                regularisees.append({'type': 'GL régularisé', 'from': r.get('carry_from',''), 'date': r.get('date',''),
                    'lib': r.get('lib',''), 'debit': r.get('debit',0), 'credit': r.get('credit',0),
...
def cloturer_mois(periode, rb_df, cp_df, erb_html_str=''):
    ...
    def prep_carry(df):
        ...
        if 'carry_from' not in df.columns: df['carry_from'] = periode
        else: df['carry_from'] = df['carry_from'].apply(lambda x: x if (x and str(x).strip() not in ('','nan','None','False')) else periode)
    ...
    st.session_state.historique.append({'periode': periode, 'n_rb': len(susp_rb), 'n_cp': len(susp_cp),
        'susp_rb': susp_rb.to_dict('records') if not susp_rb.empty else [],
        'susp_cp': susp_cp.to_dict('records') if not susp_cp.empty else [], 'erb_html': erb_html_str})
    ...
    st.session_state.mc = 0; st.session_state.s_rb = 0.0; st.session_state.s_cp = 0.0
    for _k in ['inf_pe', 'inf_cl', 'inf_bq', 'inf_co', '_inp_s_rb', '_inp_s_cp']:
...
def build_erb_html(e, info):
    ...
    for i in range(n_rows):
        ...
        if i < len(rows_right):
            pair = rows_right[i]
            d_item = pair.get('debit_item'); c_item = pair.get('credit_item')
            if d_item:
...
def export_xlsx(e, info):
    ...
    for i in range(n_rows):
        ...
        if i < len(rows_right):
            pair2   = rows_right[i]
            d_item2 = pair2.get('debit_item')
            c_item2 = pair2.get('credit_item')
...
def importer_erb_fichier(uploaded_file, periode_source):
    ...
    for row in rows[header_row+1:]:
        ...
        # ═══ CORRECTION BUG v5.6 ══════════════════════════════════════════
        # N'importer QUE les vrais suspens reportés (préfixés [↩)
        # Les items GL crédit courant (chèques émis non compensés) n'ont PAS
        # ce préfixe et ne doivent PAS être portés en carryover — ils seront
        # appariés normalement dans le nouveau mois avec les nouvelles écritures GL.
        # Sans cette correction, ils consomment à tort les écritures GL du mois
        # suivant via la passe 1b, empêchant l'appariement des vrais débits RB.
        # ═══════════════════════════════════════════════════════════════════
        if rb_lib and 'SOLDE AVANT' not in rb_lib.upper() and rb_d > 0 and '[↩' in rb_lib:
...
def page_import_erb():
    ...
    st.markdown("---")
    periode_source = st.text_input("Période du tableau ERB importé", placeholder="Ex: 31 AVRIL 2026",
    ...
    if uploaded_erb and periode_source.strip():
        ...
        with col1:
            ...
            if susp_rb:
                ...
                st.info("Aucun suspens côté Relevé")
        with col2:
            ...
            if susp_cp:
                ...
                st.info("Aucun suspens côté Journal")
        total = len(susp_rb)+len(susp_cp)
        if total == 0: st.warning("⚠️ Aucun suspens trouvé dans ce fichier ERB."); return
        if st.button(f"✅ Charger ces {total} suspens comme reports du mois précédent", type="primary", use_container_width=True):
                ...
                if has_key:
                    ...
                    if df_ocr is not None and not df_ocr.empty:
                        ...
                        df_proc = pd.DataFrame({
                            "date": df_ocr.get("date",""), "lib": df_ocr.get("libelle",""),
...
- Portail Ecobank : exportez directement en PDF numérique ou Excel""")
    ...
    for idx,field in enumerate(['date','lib','piece','debit','credit','montant','sens']):
        with cols_ui[idx%4]:
            ...
            sel=st.selectbox(FL[field],avail,index=def_idx,key=f'ms_{side}{field}')
            if sel!='— ignorer —': user_map[field]=sel
    if st.button("✅ Appliquer et importer",type="primary",key=f'btn_apply{side}'):
        ...
        if df_proc.empty:
            st.error("❌ Aucune ligne valide extraite. Vérifiez le mapping des colonnes Débit/Crédit."); return
        if side=='rb':
...
def page_mes_erb():
    st.markdown('<div class="page-title">\U0001f4da Mes ERB sauvegardes</div>', unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>Retrouvez et telechargez tous vos tableaux ERB archives</div>", unsafe_allow_html=True)
    licence_code = st.session_state.get('_licence_code','')
    sb = get_supabase()
    if sb is None:
        err_detail = st.session_state.get("_supabase_error", "")
        msg = "**Supabase non configure** \u2014 La sauvegarde cloud n'est pas encore activee."
        if err_detail: msg += f"\n\n**Erreur detaillee :** `{err_detail}`"
        st.warning(msg)
        with st.expander("SQL a executer dans Supabase", expanded=True):
            st.code("""CREATE TABLE erb_historique (
...
CREATE INDEX idx_erb_licence ON erb_historique(licence_code);""", language="sql")
        return
    with st.spinner("Chargement de votre historique..."):
        historique = charger_historique_erb(licence_code)
    if not historique:
        st.info("Aucun ERB sauvegarde pour le moment. Generez un tableau ERB pour qu'il soit automatiquement archive.")
        return
    n_ok = sum(1 for h in historique if h.get('equilibre'))
    c1, c2, c3 = st.columns(3)
    c1.metric("ERB archives", len(historique))
    ...
    for h in historique:
        ...
        with col_info:
            st.markdown(f"""<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(100,160,255,0.15);border-radius:10px;padding:12px 16px;margin-bottom:8px">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <div><span style="font-size:15px;font-weight:700;color:#fff">\U0001f4c5 {periode}</span>
    <span style="font-size:11px;color:#607D8B;margin-left:12px">Archive le {date_str}</span></div>
    <div style="background:{status_color};border-radius:6px;padding:3px 10px;font-size:12px;font-weight:700;color:#fff">{status_label}</div>
  </div>
  <div style="margin-top:8px;font-size:12px;color:#90CAF9">SR Releve : <b>{sr_rb:,.0f}</b> &nbsp;|&nbsp; SR Comptabilite : <b>{sr_cp:,.0f}</b></div>
</div>""", unsafe_allow_html=True)
        with col_dl:
            st.markdown("<div style='margin-top:12px'>", unsafe_allow_html=True)
            if st.button("\U0001f4e5", key=f"dl_{erb_id}", use_container_width=True, help="Telecharger cet ERB"):
                with st.spinner("Chargement..."):
                    xlsx_bytes, fname = telecharger_erb_sauvegarde(erb_id)
                if xlsx_bytes:
                    st.download_button(label="Telecharger", data=xlsx_bytes, file_name=fname,
...
with st.sidebar:
    ...
    if entreprise:
        ue_html = f'<div style="font-size:10px;color:rgba(163,210,230,0.6);margin-top:2px">👤 {user_email}</div>' if user_email else ''
        st.markdown(f'''<div style="background:rgba(255,255,255,0.08);border-radius:8px;padding:8px 10px;margin-bottom:8px">
  ...
  {ue_html}
    ...
    for key,icon,label in [("dashboard","📊","Tableau de bord"),("import_rb","📥","Relevé bancaire"),
            ("import_cp","📄","Écritures comptables"),("import_erb_prev","📂","ERB précédent"),
            ("rapprochement","🔗","Rapprochement"),("mes_erb","📚","Mes ERB sauvegardés")]:
        active=st.session_state.page==key
        if st.button(f"{icon} {label}",key=f"nav{key}",use_container_width=True,
    ...
    if st.button("🗑️ Réinitialiser tout", key="nav_reset_all", use_container_width=True,
        ...
        st.session_state.update(licence_bak); st.rerun()
    if st.button("🚪 Se déconnecter", key="nav_logout", use_container_width=True,
        ...
        for k in list(st.session_state.keys()): del st.session_state[k]
        st.rerun()
    st.caption("ERB v5.6 — fix importer_erb_fichier")
    ...
    if rb_for_erb is not None and len(rb_for_erb)>0 and cp_for_erb is not None and len(cp_for_erb)>0:
        ...
        if _cache_key not in st.session_state or st.session_state[_cache_key] is None:
            for _k in list(st.session_state.keys()):
                if _k.startswith(f"_xlsx_{fname.replace('.xlsx','')}"): del st.session_state[_k]
            try:
            ...
            if st.session_state.get(_cache_key):
                ...
                if not st.session_state.get(_save_key) and periode.strip():
                    ok_save, err_save = sauvegarder_erb(
                        entreprise    = st.session_state.get('entreprise',''),
                        licence_code  = st.session_state.get('_licence_code',''),
                        periode       = periode, xlsx_bytes = st.session_state[_cache_key],
                        sr_rb         = e['sr_rb'], sr_cp = e['sr_cp'], ok = e['ok'])
                    if ok_save: st.session_state[_save_key] = True
                st.download_button(
                    label="📥 Télécharger le tableau ERB (.xlsx)",
                    data=st.session_state[_cache_key], file_name=fname,
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    type="primary", use_container_width=True)
            else:
                st.button("📥 Télécharger le tableau ERB (.xlsx)", disabled=True, use_container_width=True,
                          help="Erreur de génération — vérifiez les données")
        with col_cl:
            if st.button("📅 Clôturer ce mois et reporter les suspens", use_container_width=True,

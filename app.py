...
# ── Mots-clés typiques de frais bancaires (relevé débit sans GL) ──────
_FRAIS_KEYWORDS = [
    'frais de gestion','frais gestion','frais tenue','commission banque',
    'taxe frais','taxe sur','frais telecompense','frais de traitement',
    'frais cion','frais service','frais domicil','agios','interets debiteurs',
    'commission ttc','commission hors','tva sur frais','frais carte',
    'commission sica','sica remise','frais virement','frais de virement',
    'commission virement','frais swift','frais bancaire',
    # Libellés Ecobank spécifiques (REF:Lxxx...)
    'commission - autres','autres frais','frais de gestion su',
    'commission banque centra','frais de tenue',
    # Libellés génériques courts
    'commission','agios et frais',
]

def _is_frais_bancaire(lib):
    """Retourne True si le libellé ressemble à des frais bancaires."""
    l = str(lib).lower().strip()
    return any(k in l for k in _FRAIS_KEYWORDS)

def detecter_frais_bancaires(susp_rb_debit):
    """
    Parmi les suspens RB débit (dans relevé mais pas en GL),
    identifie ceux qui sont probablement des frais bancaires non saisis.
    Retourne (frais_df, autres_df).
    """
    if susp_rb_debit is None or susp_rb_debit.empty:
        return pd.DataFrame(), pd.DataFrame()
    mask = susp_rb_debit.apply(lambda r: _is_frais_bancaire(str(r.get('lib',''))), axis=1)
    return susp_rb_debit[mask].reset_index(drop=True), susp_rb_debit[~mask].reset_index(drop=True)
...
# ── Détection chèques rejetés (carries GL crédit → col J NÉGATIF) ──
_REJETE_KEYWORDS = [
    'cheque rejete','chèque rejeté','rejet cheque','rejet chèque',
    'cheque impaye','chèque impayé','impaye','rejet cb','retour cheque',
    'cheque retourne','rejet prelevement','impayé','cheq rejete',
]

def _is_cheque_rejete(lib):
    """Retourne True si le libellé correspond à un chèque rejeté/impayé."""
    l = str(lib).lower()
...
def auto_match(rb_df, cp_df, inversion=True):
    ...
    # Régularise les carries RB qui ont une contrepartie GL (ex: annulation de chèque CBAO).
    cp_reserved = set()
    if len(rb_carry_out) > 0:
        ...
        for ci in rb_carry_out.index:
            ...
            for cj in cp_tmp.index:
                ...
                matched_inv = (carry_d > 0 and abs(carry_d - cp_c) <= 1) or \
                               (carry_c > 0 and abs(carry_c - cp_d) <= 1)
                if not matched_inv: continue
                # N'appliquer que si le libellé GL contient une référence explicite au carry
                # (annulation, même N° de pièce) — évite les faux matches sur montants courants
                cp_lib = str(cp_tmp.at[cj, 'lib']).lower()
                # Numéros de référence : ≥5 chiffres (exclut années 4 chiffres)
                nums_carry = set(w for w in __import__('re').findall(r'\d{5,}', carry_lib))
                nums_cp    = set(w for w in __import__('re').findall(r'\d{5,}', cp_lib))
                has_common_ref = bool(nums_carry & nums_cp) or \
                                 any(k in cp_lib for k in ('annul','cancel','regularise','régul'))
                if not has_common_ref: continue
                mc1b += 1; mid = f"P{mc1b}"
                rb_carry_out.at[ci, 'matched']  = True
                rb_carry_out.at[ci, 'match_id'] = mid
...
def calc_erb(rb_df, cp_df, s_rb, s_cp):
    """
    Convention ERB — règle manuelle TECHCONST/Ecobank validée.

    ═══════════════════════════════════════════════════════════════
    COL D (relevé, débit) :
      • Chèques GL courants non compensés au relevé  (susp_gl_c_courant)
      • Carries RB débit en transit                  (susp_rb_debit NON frais)
      • Carries GL crédit en transit (non rejetés)   (susp_gl_c_report NON rejeté)
      SR_RB = S_RB − Σ col_D

    COL J (journal, valeur ALGÉBRIQUE) :
      • Chèques rejetés reportés      → NÉGATIF   (−)
      • Frais bancaires RB non saisis → POSITIF   (+)
      SR_CP = S_CP − col_J_net
              = S_CP − (Σfrais+ − Σrejetés−)
              = S_CP + Σrejetés − Σfrais

    IDENTITÉ : SR_RB = SR_CP  ⟺  S_RB − S_CP = col_D − col_J_net
    ═══════════════════════════════════════════════════════════════
    """
    ...
    # Séparer frais bancaires (→ col J +) vs chèques transit (→ col D)
    if not susp_rb_debit.empty:
        mask_frais = susp_rb_debit.apply(lambda r: _is_frais_bancaire(str(r.get('lib',''))), axis=1)
        susp_rb_debit_frais   = susp_rb_debit[ mask_frais].reset_index(drop=True)
        susp_rb_debit_transit = susp_rb_debit[~mask_frais].reset_index(drop=True)
    else:
        susp_rb_debit_frais   = pd.DataFrame()
        susp_rb_debit_transit = pd.DataFrame()
    ...
    # GL crédits : courants vs reportés
    ...
    # GL crédits reportés : rejetés (→ col J −) vs transit (→ col D)
    if not susp_cp_credit_report.empty:
        mask_rej = susp_cp_credit_report.apply(
            lambda r: _is_cheque_rejete(str(r.get('lib',''))), axis=1)
        susp_cp_credit_report_rejete  = susp_cp_credit_report[ mask_rej].reset_index(drop=True)
        susp_cp_credit_report_transit = susp_cp_credit_report[~mask_rej].reset_index(drop=True)
    else:
    ...
    return dict(
        ...
        susp_rb_debit=susp_rb_debit,
        susp_rb_debit_frais=susp_rb_debit_frais,
        susp_rb_debit_transit=susp_rb_debit_transit,
        susp_rb_credit=susp_rb_credit,
...
def build_erb_html(e, info):
    """
    CONVENTION MANUELLE TECHCONST :

    COL D (relevé débit) :
      • GL crédits courants (chèques émis non compensés)
      • RB débits TRANSIT (carries + courants non-frais)
      • GL crédits reportés NON rejetés (transit d'un mois précédent)

    COL J (journal crédit, ALGÉBRIQUE) :
    ...
    for i in range(n_rows):
        ...
        # ── COTE JOURNAL (col J algébrique) ──────────────────────────
        if i < len(rows_right):
            item2 = rows_right[i]
            r2      = item2['row']
            side2   = item2['side']
            is_rep2 = item2['is_rep']
            sign2   = item2['sign']   # +1 frais, -1 rejeté
            lb2 = r2['lib']
            if is_rep2: lb2 = f'[↩ {r2.get("carry_from","")}] {lb2}'
            # Rejetés → rouge, Frais → orange sombre
            if sign2 < 0:
                s_tx2  = S['RR'];  s_num2 = S['RRR']
            else:
                s_tx2  = S['CP'];  s_num2 = S['CPR']
            # Montant brut puis signe
            montant_brut = r2['credit'] if r2['credit'] > 0 else r2['debit']
            val_affiche  = f"-{fmt_fr(montant_brut)}" if sign2 < 0 else fmt_fr(montant_brut)
            h += (f'<td style="{s_tx2}">{r2["date"]}</td><td style="{s_tx2}">{lb2}</td>'
                  f'<td style="{s_tx2}">{r2.get("piece","")}</td>'
                  f'<td style="{s_num2}"></td><td style="{s_num2}">{val_affiche}</td></tr>')
        else:
            h += (f'<td style="{S["EC"]}"></td><td style="{S["EC"]}"></td>'
                  f'<td style="{S["EC"]}"></td><td style="{S["EC"]}"></td><td style="{S["EC"]}"></td></tr>')

    # ── Totaux ───────────────────────────────────────────────────────
    col_j_net = e.get('col_j_net', 0.0)
    col_j_str = (f"-{fmt_fr(abs(col_j_net))}" if col_j_net < 0
                 else (fmt_fr(col_j_net) if col_j_net > 0 else '-'))
    h += (f'<tr><td colspan="3" style="{S["FTL"]}">TOTAUX</td>'
          ...
          f'<td style="{S["FT"]}">{col_j_str}</td></tr>')

    # SR côté relevé en DÉBIT (valeur négative conventionnelle)
    sr_rb_val = f"-{fmt_fr(e['sr_rb'])}" if e['sr_rb'] >= 0 else fmt_fr(abs(e['sr_rb']))
    h += (f'<tr><td style="{S["SR"]}"></td><td colspan="2" style="{S["SRL"]}">SOLDES RAPPROCHES</td>'
          f'<td style="{S["SR"]}"><b>{sr_rb_val}</b></td><td style="{S["SR"]}"></td>'
...
def export_xlsx(e, info):
    ...
    for i in range(n_rows):
        ...
        if i < len(rows_right):
            ...
            # Montant brut
            m_brut = r2['credit'] if r2['credit'] > 0 else r2['debit']
            val_col10 = -m_brut if sign2 < 0 else m_brut
            ws.cell(row=row,column=9,value=None).fill=cp_fl
            cl10 = ws.cell(row=row,column=10,value=val_col10)
            cl10.fill=cp_fl
            cl10.alignment=Alignment(horizontal='right')
            cl10.font=Font(bold=is_rep2, color="7A0000" if sign2<0 else "3A1800", size=10)
        else:
    ...
    col_j_net = e.get('col_j_net', 0.0)
    for c,v in [(4,e['tot_d_rb']),(5,e['tot_c_rb']),(9,e['tot_d_cp']),(10,col_j_net if col_j_net else None)]:
    ...
    # SR_RB affiché négatif (convention DEBIT = valeur négative)
    sr_rb_xlsx = -e['sr_rb'] if e['sr_rb'] >= 0 else abs(e['sr_rb'])
    ws.cell(row=row,column=4,value=sr_rb_xlsx).alignment=Alignment(horizontal='right')
...
if page=="dashboard":
    ...
    if rb_for_erb is not None and len(rb_for_erb)>0 and cp_for_erb is not None and len(cp_for_erb)>0:
        ...
        # ── AMÉLIORATION 1 : Détection des frais bancaires non saisis ──
        frais_df, autres_susp_rb = detecter_frais_bancaires(e['susp_rb_debit'])
        n_frais = len(frais_df)
        montant_frais = frais_df['debit'].sum() if not frais_df.empty else 0.0

        st.markdown("---")
        st.markdown("##### Calcul des écarts")
        ...
        if e['ok']:
            st.success("✅ Rapprochement équilibré — SR_RB = SR_CP")
        else:
            # ── Message d'écart enrichi ──────────────────────────────
            chk = e['chk']
            n_susp_rb = len(e['susp_rb'])
            n_susp_cp = len(e['susp_cp'])
            st.warning(
                f"⚠️ Écart de **{chk:,.0f} FCFA** — "
                f"{n_susp_rb} suspens relevé · {n_susp_cp} suspens GL"
            )
            ...
            if n_frais > 0:
                noms_frais = ", ".join(str(r['lib'])[:40] for _,r in frais_df.iterrows())
                causes.append(
                    f"🔴 **{n_frais} frais bancaire(s)** dans le relevé sans écriture GL "
                    f"— montant : **{montant_frais:,.0f} FCFA**"
                )
                for _,r in frais_df.iterrows():
                    actions.append(
                        f"• Saisir en GL : `{r['date']}` — _{r['lib']}_ — **{r['debit']:,.0f} FCFA**"
                    )

            n_autres_rb = len(autres_susp_rb)
            if n_autres_rb > 0:
                causes.append(
                    f"🟡 **{n_autres_rb} suspens relevé** non identifiés comme frais "
                    f"(chèques en transit ou erreurs de montant)"
                )

            n_susp_gl_c = len(e.get('susp_cp_credit_courant', pd.DataFrame()))
            if n_susp_gl_c > 0:
                gl_sum = e.get('susp_cp_credit_courant', pd.DataFrame())
                if not gl_sum.empty:
                    gl_sum_v = gl_sum['credit'].sum()
                    causes.append(
                        f"🟡 **{n_susp_gl_c} chèque(s) GL** émis non encore compensés au relevé "
                        f"— montant : **{gl_sum_v:,.0f} FCFA** (normal si récent)"
                    )

            if causes:
                with st.expander("🔍 Diagnostic de l'écart — Actions recommandées", expanded=True):
                    st.markdown("**Causes identifiées :**")
                    for c in causes: st.markdown(c)
                    if actions:
                        st.markdown("---")
                        st.markdown("**Actions à mener par le comptable :**")
                        for a in actions: st.markdown(a)
                        st.markdown(
                            f"Une fois ces **{n_frais} écriture(s)** saisie(s) en GL "
                            f"et le fichier re-importé, le statut passera à **OK ✅**."
                        )
        ...
        if n_susp_rb_courant > 0 and not e['ok']:
            # Filtrer : exclure les compensations de chèques (déjà un GL proche)
            # Ne garder que frais, droits, taxes — pas les REGLEMENT COMPENSATION
            _EXCL_TRANSIT = ['reglement compensation','compensation cheque',
                             'remise cheque','versement','retrait par cheque',
                             'ret chq','retrait chq']
            susp_rb_integrables = susp_rb_d_courant[
                ~susp_rb_d_courant['lib'].apply(
                    lambda l: any(k in str(l).lower() for k in _EXCL_TRANSIT)
                )
            ].copy()
            n_integrables = len(susp_rb_integrables)
            total_integrables = susp_rb_integrables['debit'].sum() if n_integrables > 0 else 0

            if n_integrables > 0:
                st.info(
                    f"⚡ **{n_integrables} écriture(s) GL manquante(s) détectée(s)** "
                    f"(frais, droits, taxes) — total **{total_integrables:,.0f} FCFA**\n\n"
                    f"L'outil peut les pointer automatiquement côté Journal (crédit) :"
                )
                col_btn1, col_btn2 = st.columns([2,1])
                with col_btn1:
                    if st.button(
                        f"⚡ Intégrer {n_integrables} écriture(s) → GL crédit",
                        key="btn_prefill_all_top", type="primary", use_container_width=True
                    ):
                        lignes_init = []
                        for _, r in susp_rb_integrables.iterrows():
                            lignes_init.append({
                                'date':   r.get('date',''),
                                'lib':    r.get('lib',''),
                                'piece':  r.get('piece',''),
        ...
        with st.expander("➕ Saisir des écritures GL manquantes (frais bancaires, régularisations…)", expanded=(n_susp_rb_courant > 0 and not e['ok'])):
            ...
            if n_frais > 0 and not e['ok']:
                ...
                if st.button("⚡ Pré-remplir depuis frais relevé", key="btn_prefill_frais"):
                    lignes_init = []
                    for _,r in frais_df.iterrows():
                        lignes_init.append({
                            'date':   r.get('date',''),
            ...
            if '_gl_manual_rows' not in st.session_state:
                st.session_state['_gl_manual_rows'] = []

            rows_to_add = st.session_state['_gl_manual_rows']

            col_h1,col_h2,col_h3,col_h4,col_h5,col_h6 = st.columns([2,4,2,2,2,1])
            col_h1.markdown("**Date**"); col_h2.markdown("**Libellé**")
            col_h3.markdown("**N° Pièce**"); col_h4.markdown("**Débit GL**")
            col_h5.markdown("**Crédit GL**"); col_h6.markdown("")

            rows_updated = []
            for idx, row_val in enumerate(rows_to_add):
                c1,c2,c3,c4,c5,c6 = st.columns([2,4,2,2,2,1])
                with c1: d  = st.text_input("",  value=row_val.get('date',''),  key=f"_glm_d{idx}",  label_visibility='collapsed', placeholder="JJ/MM/AAAA")
                with c2: lb = st.text_input("",  value=row_val.get('lib',''),   key=f"_glm_l{idx}",  label_visibility='collapsed', placeholder="Libellé")
                with c3: pc = st.text_input("",  value=row_val.get('piece',''), key=f"_glm_p{idx}",  label_visibility='collapsed', placeholder="Réf.")
                with c4: dv = st.number_input("",value=float(row_val.get('debit',0)),  min_value=0.0, step=1.0, format="%.0f", key=f"_glm_dv{idx}", label_visibility='collapsed')
                with c5: cv = st.number_input("",value=float(row_val.get('credit',0)), min_value=0.0, step=1.0, format="%.0f", key=f"_glm_cv{idx}", label_visibility='collapsed')
                with c6:
                    if st.button("🗑", key=f"_glm_del{idx}", help="Supprimer cette ligne"):
                        continue  # sauter = supprimer
                rows_updated.append({'date':d,'lib':lb,'piece':pc,'debit':dv,'credit':cv})
            st.session_state['_gl_manual_rows'] = rows_updated

            btn_c1, btn_c2, btn_c3 = st.columns([1,1,2])
            with btn_c1:
                if st.button("➕ Nouvelle ligne", key="btn_add_gl_manual", use_container_width=True):
                    st.session_state['_gl_manual_rows'].append({'date':'','lib':'','piece':'','debit':0.0,'credit':0.0})
                    st.rerun()
            with btn_c2:
                if st.button("🗑 Tout effacer", key="btn_clear_gl_manual", use_container_width=True):
                    st.session_state['_gl_manual_rows'] = []; st.rerun()
            with btn_c3:
                lignes_valides = [r for r in rows_updated if r['lib'].strip() and (r['debit']>0 or r['credit']>0)]
                if st.button(
                    f"✅ Intégrer {len(lignes_valides)} écriture(s) au GL et relancer",
                    key="btn_apply_gl_manual",
                    type="primary", use_container_width=True,
                    disabled=(len(lignes_valides) == 0)
                ):
                    if st.session_state.cp_df is not None:
                        nouvelles = pd.DataFrame([{
                            ...
                            'matched': False,
                            'match_id':None,
                        } for r in lignes_valides])
                        st.session_state.cp_df = pd.concat(
                            [st.session_state.cp_df, nouvelles], ignore_index=True
                        )
                        # Relancer le rapprochement automatique
                        rb_full2 = get_rb_with_carryover(); cp_full2 = get_cp_with_carryover()
                        rb_full2 = rb_full2.copy(); cp_full2 = cp_full2.copy()
                        rb_full2['matched']=False; rb_full2['match_id']=None
                        cp_full2['matched']=False; cp_full2['match_id']=None
                        st.session_state.mc = 0
                        rb_m2, cp_m2, cnt2 = auto_match(rb_full2, cp_full2, inversion=st.session_state.get('inv_tog',True))
                        st.session_state.regularisees = extraire_regularisees(rb_m2, cp_m2)
                        if 'carried' in rb_m2.columns:
                            st.session_state.rb_df = rb_m2[~rb_m2['carried'].fillna(False).astype(bool)].drop(columns=['carried'],errors='ignore').copy()
                            carried_rb2 = rb_m2[rb_m2['carried'].fillna(False).astype(bool)].copy()
                            non_reg2 = carried_rb2[~carried_rb2['matched'].fillna(False).astype(bool)].drop(columns=['carried'],errors='ignore').copy()
                            if len(non_reg2)>0: st.session_state.rb_df = pd.concat([st.session_state.rb_df, non_reg2], ignore_index=True)
                            st.session_state.carryover_rb = []
                        else: st.session_state.rb_df = rb_m2
                        if 'carried' in cp_m2.columns:
                            st.session_state.cp_df = cp_m2[~cp_m2['carried'].fillna(False).astype(bool)].drop(columns=['carried'],errors='ignore').copy()
                            carried_cp2 = cp_m2[cp_m2['carried'].fillna(False).astype(bool)].to_dict('records')
                            st.session_state.carryover_cp = [r for r in carried_cp2 if not r.get('matched')]
                        else: st.session_state.cp_df = cp_m2
                        for _k in list(st.session_state.keys()):
                            if _k.startswith('_xlsx_'): del st.session_state[_k]
                        st.session_state['_gl_manual_rows'] = []
                        st.session_state['_rematch_msg'] = f"✅ {len(lignes_valides)} écriture(s) GL ajoutée(s) · {cnt2} rapprochements"
                        st.rerun()
                    else:
                        st.error("Importez d'abord le journal comptable.")

        # ── Tableau ERB ──────────────────────────────────────────────
        st.markdown("---")
        st.markdown("##### 📋 Tableau ERB — Aperçu avant export")
